...
- **`fraggle-agent-core`** - Generic agent runtime: `Agent`, `runAgentLoop`, `AgentState`, `AgentEvent`, `AgentMessage`, `LlmBridge`/`ProviderLlmBridge`, `AgentToolDef`, `FraggleToolRegistry`, `SupervisedToolCallExecutor`, `ToolExecutor`/`LocalToolExecutor`, `ToolSupervisor` + policy rules, `RemoteToolClient`, `AgentEventTracer`, `TraceStore`, `EventBus`, `ResponseAttachment`, `ToolExecutionContext`, **the shared compaction layer** (`Compactor`, `LlmCompactor`, `CompactionPolicy`, `ContextUsage`), **and skill Python/secrets support** (`SkillSecretsStore`, `SkillVenvManager`, `SkillExecutionContext`/`DefaultSkillExecutionContext`). Reusable by any agent application. `AgentCoreModule` provides `LMStudioProvider`, `ToolExecutor`, `ToolSupervisor`, `RemoteToolClient?`, `TraceStore?`, `SkillSecretsStore`, `SkillVenvManager`, `SkillExecutionContext`.
